...
void dtGoToBehavior::update(dtCrowdAgent* ag, float* force, const float dt) const
{
	if (!m_target)
		return;
	...
	dtVsub(desiredForce, m_target, ag->npos);
	...
	float distToObj = dtVdist(ag->npos, m_target) - m_distance;
