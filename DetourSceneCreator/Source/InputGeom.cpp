...
bool InputGeom::loadMesh(class rcContext* ctx, float* vert, unsigned vertCount, int* tris, unsigned triCount)
{
	if (m_mesh)
	{
		delete m_chunkyMesh;
	...
	if (!m_mesh->load(vert, vertCount, tris, triCount))
	{
		ctx->log(RC_LOG_ERROR, "buildTiledNavigation: Could not load the mesh using arrays");
		return false;
	}
